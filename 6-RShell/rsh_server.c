...
int boot_server(char *ifaces, int port)
{
    ...
    // Allows linux to reusue the port if you dont clean it up properly
    int enable = 1;
    setsockopt(svr_socket, SOL_SOCKET, SO_REUSEADDR, &enable, sizeof(int));

    //  2. Calling bind to "bind" the server to the interface and port
    addr.sin_family = AF_INET;
    // In almost all cases ifaces will just be 0.0.0.0 but incase its not
    // Set the interface to the correct format address.
...
int process_cli_requests(int svr_socket)
{
    ...
    while (1)
    {
        // DONE use the accept syscall to create cli_socket
...
int exec_client_requests(int cli_socket)
{
    ...
    if (recv_buff == NULL || cmd_list == NULL)
    {
        printf(CMD_ERR_RDSH_ITRNL, ERR_MEMORY);
        return ERR_MEMORY;
    }

    while (1)
    {
        // DONE use recv() syscall to get input
        ret = recv(cli_socket, recv_buff, RDSH_COMM_BUFF_SZ, 0);
        if (ret == -1)
        {
            perror("recv");
            printf(CMD_ERR_RDSH_COMM);
        ...
        printf(RCMD_MSG_SVR_EXEC_REQ, recv_buff);
        // DONE build up a cmd_list
        cmd_rc = build_cmd_list(recv_buff, cmd_list);

        // DONE rsh_execute_pipeline to run your cmd_list
        last_rc = rsh_execute_pipeline(cli_socket, cmd_list);
        send_message_eof(cli_socket);
        printf(RCMD_MSG_SVR_RC_CMD, last_rc);
        switch (last_rc)
        {
        case 1:
        ...
        case ERR_EXEC_CMD:
            printf(CMD_ERR_RDSH_EXEC);
            send_message_string(cli_socket, RCMD_MSG_SVR_RC_CMD);
            continue;
        default:
...
{
    int rc;
    // DONE implement writing to cli_socket with send()
    size_t msg_len = strlen(buff);
    rc = send(cli_socket, buff, msg_len, 0);
    if (rc == -1)
    {
        perror("send");
        return ERR_RDSH_COMMUNICATION;
...
int rsh_execute_pipeline(int cli_sock, command_list_t *clist)
{
    ...
    pid_t pids[clist->num];       // Store process IDs
    int pids_st[clist->num];      // Store exit statuses
    ...
    for (int i = 0; i < clist->num; i++)
    {
        ...
        case BI_CMD_STOP_SVR:
            return OK_EXIT;
        case BI_CMD_DRAGON:
            // Fork a process for dragon command
            pids[i] = fork();
            if (pids[i] == -1)
            {
                perror("fork");
            ...
            { // Child process
                if (i == 0)
                {
                    dup2(cli_sock, STDIN_FILENO); // First process reads from socket
                }
                else
                {
                ...
                if (i == clist->num - 1)
                {
                    dup2(cli_sock, STDOUT_FILENO); // Last process writes to socket
        ...
        case BI_NOT_BI:
            ...
            // HINT you can dup2(cli_sock with STDIN_FILENO, STDOUT_FILENO, etc.
            if (pids[i] == 0)
            { // Child process
                if (i == 0)
                {
            ...
            break; // Continue to next process
        }
    }
