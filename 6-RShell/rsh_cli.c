...
int exec_remote_cmd_loop(char *address, int port)
{
    ...
    // DONE set up cmd and response buffs
    ...
    while (1)
    {
        ...
        // DONE send() over cli_socket
        io_size = strlen(cmd_buff) + 1;
        ret = send(cli_socket, cmd_buff, io_size, 0);
        if (ret == -1)
        {
            perror("send");
        ...
        while (1)
        {
            ...
            {
                // Print recieved message to client terminal
                printf("%.*s", (int)recv_bytes, rsp_buff);
                // Check if the last received byte is EOF
        ...
        // DONE break on exit command
        if (strncmp(cmd_buff, "exit", 4 * sizeof(char)) == 0 || strncmp(cmd_buff, "stop-server", 11 * sizeof(char)) == 0)
        {
            break;
        }
    }
